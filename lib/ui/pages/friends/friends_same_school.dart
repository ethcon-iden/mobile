...
class FriendsSameSchool extends StatefulWidget {
  const FriendsSameSchool({Key? key}) : super(key: key);

  @override
  State<FriendsSameSchool> createState() => _FriendsSameSchoolState();
}

class _FriendsSameSchoolState extends State<FriendsSameSchool> {
  ...
  void _getSearch() async {
    ...
    HttpsResponse res = await RelationshipApi.getFollowSameSchool(paging: paging, name: name);
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Scaffold(
        ...
        appBar: kStyle.appBar(context, '같은 학교 친구들'),
  ...
  Widget _buildItems(List<dynamic> users) {
    return NotificationListener<ScrollNotification>(
      ...
      child: _animatedList(users),
    );
  }

  Widget _animatedList(List<dynamic> users) {
    print('---> animated list > users len: ${users.length}');
    return AnimatedList(
        key: _animatedKey,
